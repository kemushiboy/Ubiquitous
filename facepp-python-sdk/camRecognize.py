...
API_KEY = 'd471343b9c6b09dcc46cc4d6b862a3ab'
API_SECRET = 'tkWuDEM57KnlD3tY_blJfZVSLJKeyinh'
...
PERSONS = [('person', '/Users/Ishikawa/recognize/' + IMAGE_DIR)]
...
# 步骤4：训练模型
    rst = api.train.search(faceset_name = 'cam1')
...
# 步骤5：识别新图中的Face
    ...
    rst = api.recognition.search(key_face_id = faceIDs[0], faceset_name = 'cam1')
    cam1Faceid = rst["candidate"][0]["face_id"]
    cam1Similarity = rst["candidate"][0]["similarity"]
    print cam1Faceid
    print cam1Similarity
...
#search DIR by face_id
    csv1  = open("/Users/Ishikawa/cam1.csv",'r')
...
#   CSVdict3 = csv.Reader(open("/Users/Ishikawacam3.csv",'r'), delimiter=',')
    ...
    for line in csv1:
        if cam1Faceid in line:
            text = line.strip(cam1Faceid)
            text = text.strip(",")
            text = text.strip(".jpg\n")
...
#send OSC message
    ...
    msg = OSC.OSCMessage()
    msg.setAddress("/recognize")
    if cam1Similarity > 60:
        msg.append(frame1)
    else:
        msg.append("0")
